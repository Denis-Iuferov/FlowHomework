...
class SampleInteractor(
    ...
    fun task2(): Flow<String> {
        return sampleRepository.produceNumbers().transform { value ->
            emit(value.toString())
            val isDivisibleBy3 = value % 3 == 0
            val isDivisibleBy5 = value % 5 == 0
            val isDivisibleBy15 = isDivisibleBy3 && isDivisibleBy5
            when {
                isDivisibleBy15 -> emit("FizzBuzz")
                isDivisibleBy3 -> emit("Fizz")
                isDivisibleBy5 -> emit("Buzz")
            }
        }
    }
